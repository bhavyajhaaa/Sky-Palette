...
type Settings = {
 ...
 trailSpacing:number; paused:boolean;
};

const defaults:Settings={quality:'medium',ambientSpeed:.055,ambientStrength:.045,pointerForce:.095,vorticity:.075,rippleStrength:.014,splatRadius:.205,persistence:14,trailSpacing:48,paused:false};
...
void main(){
 ...
 if(motion>.5){
  ...
  for(int i=0;i<12;i++){
   ...
   vec2 center=stirs[i].xy+velocity*age*pointerForce*.8,d=p-center;d.x*=aspect;
   float r=length(d)+.003,speed=min(2.,length(velocity));
   float fall=exp(-(r*r)/(splatRadius*splatRadius))*pow(life,.55);
   vec2 radial=d/r,tangent=vec2(-radial.y,radial.x);
   float rotation=sin(age*10.+r*9.)*.5+.85;
   p+=tangent*fall*vorticity*(.35+speed)*rotation;
   p-=velocity*fall*pointerForce*exp(-age*2.2);
   p+=radial*sin(r*58.-age*30.)*fall*rippleStrength*(.5+speed);
  }
 }
 p=clamp(p,vec2(.002),vec2(.998));gl_FragColor=texture2D(palette,p);
}`;
...
export function FluidColorField({colors}:{colors:string[]}){
 ...
 return <div className="relative h-[calc(100dvh-4rem)] overflow-hidden bg-neutral-500"><canvas ref={canvas} className="block h-full w-full touch-none" aria-label="Interactive mesh gradient made from archived sky colors" onPointerMove={e=>stir(e.clientX,e.clientY)} onPointerDown={e=>{const r=e.currentTarget.getBoundingClientRect();last.current={x:(e.clientX-r.left)/r.width,y:1-(e.clientY-r.top)/r.height,time:performance.now()}}}/>
  {panel&&<aside aria-label="Fluid settings" className="absolute right-4 top-4 z-20 max-h-[calc(100%-4.5rem)] w-[min(330px,calc(100vw-2rem))] overflow-auto bg-[#111]/95 text-white shadow-2xl backdrop-blur-md"><div className="flex items-center justify-between px-4 py-3"><span className="site-mark text-[11px] font-semibold tracking-[.14em]">FLUID SETTINGS</span><button onClick={()=>setPanel(false)} className="text-lg text-white/60">×</button></div><label className="grid grid-cols-[1fr_120px] items-center border-t border-white/10 px-4 py-2.5 text-[11px] text-white/75"><span>render quality</span><select value={settings.quality} onChange={e=>set('quality',e.target.value as Quality)} className="bg-white/10 px-2 py-1.5 text-white"><option className="text-black">low</option><option className="text-black">medium</option><option className="text-black">high</option></select></label>
   <Range label="ambient speed" value={settings.ambientSpeed} min={0} max={.15} step={.005} onChange={v=>set('ambientSpeed',v)}/><Range label="ambient strength" value={settings.ambientStrength} min={0} max={.12} step={.005} onChange={v=>set('ambientStrength',v)}/><Range label="pointer force" value={settings.pointerForce} min={0} max={.25} step={.005} onChange={v=>set('pointerForce',v)}/><Range label="vorticity / curl" value={settings.vorticity} min={0} max={.2} step={.005} onChange={v=>set('vorticity',v)}/><Range label="ripple strength" value={settings.rippleStrength} min={0} max={.05} step={.002} onChange={v=>set('rippleStrength',v)}/><Range label="splat radius" value={settings.splatRadius} min={.04} max={.4} step={.005} onChange={v=>set('splatRadius',v)}/><Range label="persistence (seconds)" value={settings.persistence} min={2} max={30} step={1} onChange={v=>set('persistence',v)}/><Range label="trail spacing (ms)" value={settings.trailSpacing} min={16} max={140} step={4} onChange={v=>set('trailSpacing',v)}/><label className="flex items-center justify-between border-t border-white/10 px-4 py-3 text-[11px] text-white/75"><span>paused</span><input type="checkbox" checked={settings.paused} disabled={reduced} onChange={e=>set('paused',e.target.checked)} className="accent-sky-500"/></label><button onClick={restore} className="w-full border-t border-white/10 px-4 py-3 text-left text-[11px] text-sky-300">Restore defaults</button></aside>}
